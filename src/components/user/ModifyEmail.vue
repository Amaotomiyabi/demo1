<template>
  <div class="container">
    <el-steps :active="active" finish-status="success" :space="200" style="margin-left:calc(50% - 350px/2);">
    ...
    <div class="form-container" v-if="active==0">
      <el-form :model="form1" status-icon :rules="rules1" ref="form1" label-width="100px" class="form1">
        ...
        <el-form-item label="新邮箱" prop="newEmail">
          <el-input type="text" v-model="form1.newEmail" autocomplete="off" class="form-inpu"></el-input>
        </el-form-item>
      </el-form>
    ...
    <div class="form-container" v-if="active==1">
      <el-form :model="form2" status-icon ref="form2" label-width="100px" class="form1">
        <el-form-item label="验证码" prop="validateCode">
          <el-input type="text" v-model="form2.validateCode" autocomplete="off" class="form-inpu" ref="form2Code">
          </el-input>
        </el-form-item>
      </el-form>
      <div class="option-container">
  ...
  export default {
    ...
    methods: {
      ...
      sendOldCode() {
        ...
        Network.post('/user/msg/mail/change/old').then(data => {
          if (data.code != '200') {
            this.disabled1 = false
      ...
      submitForm1(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            Network.post('/user/identity/email/old/validate', {
              code: this.form1.validateCode,
              email: this.form1.newEmail
            }).then(data => {
              if (data.code == '200') {
                this.next()
              } else {
      ...
      submitForm2(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            Network.post('/user/identity/email/new/modify', {
              code: this.form2.validateCode,
              email: this.form1.newEmail
            }).then(data => {
              if (data.code == '200') {
                this.next()
